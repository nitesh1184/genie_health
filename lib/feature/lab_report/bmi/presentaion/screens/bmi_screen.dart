  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: PreferredSize(
        ...
        child: TopBar(username: 'BMI Screening', onBack: context.pop,),
      ),
          ...
          if (state is BmiLoading) {
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  PatientInfoCard(isExpanded: patientCubit.isExpanded,
                    onToggleExpand: patientCubit.toggleExpandCollapse,), // 🧩 Reusable card
                  const SizedBox(height: 16),
                  const Text('BMI Screening', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 8),
                  Row(
                    children: [
                      Expanded(child: _buildTextField('Weight (kg)', weightController)),
                      const SizedBox(width: 16),
                      Expanded(child: _buildTextField('Height (cm or feet/inch)', heightController)),
                    ],
                  ),
                const SizedBox(height: 8),
                _buildTextField('BMI( Kg/m2)', bmiController),
                  const SizedBox(height: 16),
                  _buildMultilineTextField('Notes (Optional)',notesController),
                  const SizedBox(height: 24),
                  const Text('Additional Body Composition Inputs', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 8),
      ...
      children: [
        ...
        Row(
          children: [
            ...
            const SizedBox(width: 16),
            Expanded(child: _buildTextField('Muscle Mass', muscleMassController)),
          ],
        ),
        const SizedBox(height: 16),
        Row(
          children: [
            Expanded(child: _buildTextField('Visceral Fat Level', visceralFatController)),
            const SizedBox(width: 16),
            Expanded(child: _buildTextField('Metabolic Age', metabolicAgeController)),
          ],
        ),
      ],
    );
  ...
  Map<String, dynamic> _prepareBmiRequest() {
    return {
      ...
      "parameters": [
        ...
        _parameter("comments", notesController.text, "None"),
      ],
    };
  }
